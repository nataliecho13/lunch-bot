...
const KV_PRECRON_MANUAL_DAY_ET_KEY = 'lunch_bot:precron_manual_day_et';
const KV_CRON_LUNCH_POSTED_DAY_ET_KEY = 'lunch_bot:cron_lunch_posted_day_et';

/** YYYY-MM-DD in America/New_York (for Redis date keys). */
export function getSuggestionDayKeyET() {
  const now = new Date();
...
/**
 * Tue/Thu, 9:30–10:29 America/New_York (inclusive). One hour ending before the 10:30 ET cron.
 * Mentions here set a flag so that day’s scheduled cron is skipped.
 */
export function isPreCronMentionWindowET() {
  const now = new Date();
 ...
 */
export async function hasPreCronManualTodayET(kv) {
  if (!kv) return false;
  try {
    const stored = await kv.get(KV_PRECRON_MANUAL_DAY_ET_KEY);
    return stored === getSuggestionDayKeyET();
  } catch {
    return false;
  }
}
 ...
 */
export async function markPreCronManualDayET(kv) {
  if (!kv) return;
  try {
    await kv.set(KV_PRECRON_MANUAL_DAY_ET_KEY, getSuggestionDayKeyET());
  } catch {}
}
 ...
 * @returns {Promise<boolean>}
 */
export async function hasCronLunchPostedTodayET(kv) {
  if (!kv) return false;
  try {
    const stored = await kv.get(KV_CRON_LUNCH_POSTED_DAY_ET_KEY);
    return stored === getSuggestionDayKeyET();
  } catch {
    return false;
  }
 ...
 * @param {{ set: (k: string, v: string) => Promise<void> }} kv
 */
export async function markCronLunchPostedTodayET(kv) {
  if (!kv) return;
  try {
    await kv.set(KV_CRON_LUNCH_POSTED_DAY_ET_KEY, getSuggestionDayKeyET());
  } catch {}
}
