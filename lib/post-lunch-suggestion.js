 ...
 *   threadTs?: string,
 *   recordPreCronManualDay?: boolean,
 * }} [params.options]
 */
export async function postLunchSuggestion({ slack, channel, kvClient, options = {} }) {
  ...
  const result = await slack.chat.postMessage({
    channel,
    thread_ts: threadTs,
    // Match scheduled posts: @mention replies also show in the main channel timeline.
    ...(threadTs ? { reply_broadcast: true } : {}),
    text,
    blocks,
  });

  if (recordPreCronManualDay) {
    await markPreCronManualDayET(kvClient);
  }
